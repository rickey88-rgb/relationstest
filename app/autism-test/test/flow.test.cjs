...
assert(a.text().includes('En kombination i dina svar sticker ut'));
...
const checkout=a.button('Visa min fullständiga analys');checkout.props.onClick();checkout.props.onClick();a.render();
...
assert(!a.text().includes('Visa min fullständiga analys'));assert.equal(a.data.get(key+':paid'),'true');
...
a.env.blocked=true;a.click('Visa min fullständiga analys');assert(!a.events.some(e=>e[0]==='checkout'));assert(a.text().includes('kunde sparas säkert'));
