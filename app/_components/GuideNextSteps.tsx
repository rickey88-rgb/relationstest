...
export default function GuideNextSteps({ sourcePage }: { sourcePage: string }) {
  ...
  return <section aria-labelledby="guide-next-steps" className="mt-12 min-w-0 rounded-2xl border border-neutral-200 bg-neutral-50 p-5 text-neutral-900 sm:p-7">
    ...
    {related.length > 0 && <div className="mt-5">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">{related.map((link, index) => <Link key={link.href} href={link.href} onClick={() => trackEvent("related_guide_click", { source_page: sourcePage, destination_page: link.href, position: index + 1 })} className="block min-w-0 rounded-xl border border-neutral-200 bg-white p-4 hover:border-neutral-300 hover:bg-neutral-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#2F6B4F]">
    ...
    {item.test && <div className="mt-6 border-t border-neutral-200 pt-5">
      ...
      <Link href={testConfig[item.test.testId].path} onClick={() => trackEvent("guide_to_test_click", { source_page: sourcePage, destination_test: item.test?.testId, cta_label: item.test?.label })} className="mt-4 inline-flex min-h-12 w-full items-center justify-center rounded-xl bg-[#2F6B4F] px-5 py-3 text-center font-semibold text-white hover:bg-[#285C44] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#2F6B4F] sm:w-auto">{item.test.label}</Link>
    </div>}
  </section>;
}
