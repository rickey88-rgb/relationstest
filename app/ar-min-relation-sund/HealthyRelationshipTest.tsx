...
export default function HealthyRelationshipTest() {
  ...
  if (view === "result" && result) return <div data-nosnippet className="mt-8 space-y-8">
    ...
    <section data-flow="card" className={panel} aria-labelledby="healthy-next-heading"><h2 id="healthy-next-heading" className="text-2xl font-semibold">Relevanta nästa steg</h2><p className="mt-3 leading-7 text-neutral-700">{result.weaker.length ? "Länkarna väljs utifrån de svagare områdena i dina svar." : "Dina svar ger ingen anledning att fylla på med problemtester. Här är en neutral fördjupning om sunda relationer."}</p><div className="mt-5 grid gap-4 sm:grid-cols-2">{result.recommendations.map((recommendation, position) => <Link data-flow="inset" key={recommendation.href} href={recommendation.href} onClick={() => trackEvent("healthy_relationship_specialist_click", { target_route: recommendation.href, destination_test: recommendation.target, weakest_dimension: result.weaker[0], recommendation_position: position + 1, position: position + 1 })} className="rounded-xl border border-neutral-200 p-5 transition-colors hover:border-neutral-400 hover:bg-neutral-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-neutral-900"><h3 className="font-semibold">{recommendation.title}</h3><p className="mt-2 text-sm leading-6 text-neutral-600">{recommendation.description}</p><span className="mt-4 inline-block font-semibold underline underline-offset-4">Läs vidare</span></Link>)}</div></section>
    <ShareTest
      heading="Vill du skicka testet till någon annan?"
      description="Du delar bara länken till testet – aldrig dina egna svar eller ditt resultat."
      shareTitle="Är min relation sund?"
      shareText="Ett kort relationstest om trygghet, respekt, kommunikation och andra viktiga områden."
      shareUrl="https://www.relationsvarning.se/ar-min-relation-sund?utm_source=share&utm_medium=referral&utm_campaign=healthy_relationship_test"
      clickEvent="healthy_relationship_share_click"
      successEvent="healthy_relationship_share_success"
    />
...
function DimensionCard({ dimension, score, tone }: { dimension: keyof typeof dimensionLabels; score: number; tone: "strength" | "attention" }) {
  return <div data-flow="inset" className="rounded-xl bg-neutral-50 p-4"><div className="flex items-center justify-between gap-4"><h3 className="font-semibold">{dimensionLabels[dimension]}</h3><span className="shrink-0 text-sm tabular-nums text-neutral-600">{score} av 6</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-neutral-200" aria-hidden="true"><div data-flow="fill" className={`h-full rounded-full ${tone === "strength" ? "bg-[#2F6B4F]" : "bg-amber-700"}`} style={{ width: `${score / 6 * 100}%` }} /></div><p className="mt-3 text-sm leading-6 text-neutral-700">{dimensionDescriptions[dimension]}</p></div>;
}
